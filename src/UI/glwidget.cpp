...
/**
   ...
   @return aucun
*/
void a2ri_vf_vertexArray(vf_model *m, double *vertexArray)
{
  int indice = 0;
...
/**
   ...
   @return aucun
*/
void a2ri_vf_indexArray(vf_model *m, int *indexArray)
{
  int indice = 0;
...
/**
   ...
   @return aucun
*/
void a2ri_vf_VertexNormalArray(vf_model *m, double *normalArray)
{
...
/**
   ...
   @return aucun
*/
void a2ri_vf_FaceNormalArray(vf_model *m, double *normalArray)
{
  for (int i = 0; i < m->nbface; i++)
  {
    vector3d norm;
...
/**
   ...
   @return aucun
*/
void a2ri_vf_normal_vector_face(vf_model *m, int numfa, double factor,
...
/**
   ...
   @return aucun
*/
void a2ri_vf_FaceNormalVectorArray(vf_model *m, double *normalVectorArray, double *factorNormalArray)
{
  int index = 0;
    ...
    : QOpenGLWidget(parent)
{
  ...
  format.setVersion(3, 2);                        // Set the OpenGL version
  format.setProfile(QSurfaceFormat::CompatibilityProfile); // Set the OpenGL profile
...
void GLWidget::paintGL()
{
  ...
  glClearColor(colorBG.redF(), colorBG.greenF(), colorBG.blueF(), colorBG.alphaF());
...
void GLWidget::polyChangeMode(GLenum polyMode)
{
...
void GLWidget::updateInfosModel()
{
  if (open)
  {
    if (nameInfos != NULL)
...
void GLWidget::display_triangle() {
  ...
  if (lightingEnabled && isDisplayFaceNormal) {
    // Disable lighting temporarily
    glDisable(GL_LIGHTING);

    // Set color for normals
    glColor4f(colorNorm.redF(), colorNorm.greenF(), colorNorm.blueF(), colorNorm.alphaF());

    // Draw face normals as lines
    glBegin(GL_LINES);
    for (int i = 0; i < model.nbface; ++i) {
      glVertex3dv(&tabFaceNormal[i * 3]);
      glVertex3d(tabFaceNormal[i * 3] + tabFactorNormal[i * 3] * tabFaceNormal[i * 3],
                  tabFaceNormal[i * 3 + 1] + tabFactorNormal[i * 3 + 1] * tabFaceNormal[i * 3 + 1],
                  tabFaceNormal[i * 3 + 2] + tabFactorNormal[i * 3 + 2] * tabFaceNormal[i * 3 + 2]);
    }
    glEnd();
  ...
  if (buf_index != 0) {
    ...
    if (fillmode) {
        ...
        glColor4f(colorOb.redF(), colorOb.greenF(), colorOb.blueF(), colorOb.alphaF());
        glDrawElements(GL_TRIANGLES, model.nbface * 3, GL_UNSIGNED_INT, nullptr);
    ...
    if (linemode) {
      ...
      glColor4f(colorL.redF(), colorL.greenF(), colorL.blueF(), colorL.alphaF());
      glDrawElements(GL_TRIANGLES, model.nbface * 3, GL_UNSIGNED_INT, nullptr);
    ...
    if (pointmode) {
      ...
      glColor4f(colorP.redF(), colorP.greenF(), colorP.blueF(), colorP.alphaF());
      glDrawElements(GL_TRIANGLES, model.nbface * 3, GL_UNSIGNED_INT, nullptr);
      if (lightingEnabled) {
          glEnable(GL_LIGHTING);
      }
...
int GLWidget::openModel(const QString &filename)
{
  if (open)
  {
    closeModel();
...
void GLWidget::changeSizePoint(double size)
{
  pSize = size;
  update();
}

void GLWidget::changeSizeLine(double size)
{
  lSize = size;
  update();
}

void GLWidget::changeBgColor(QColor color)
{
  colorBG = color;
  glColor4f(colorBG.redF(), colorBG.greenF(), colorBG.blueF(), colorBG.alphaF());
  update();
}

void GLWidget::changeObColor(QColor color)
{
  colorOb = color;
  update();
}
...
void GLWidget::changePointColor(QColor color)
{
  colorP = color;
  update();
}

void GLWidget::setDisplayFaceNormal(bool a)
{
...
void GLWidget::changeNormColor(QColor color)
{
  colorNorm = color;
  update();
}
void GLWidget::changeSizeNormLine(double size)
{
...
void GLWidget::changeTriangleOrientation(GLenum mode)
{
  // si le mode change
...
void GLWidget::saveScreenshot(const QString &text, int resolution)
{
...
void GLWidget::saveModel(QString filename)
{
  char *name = new char[filename.size() + 1];
  strcpy(name, filename.toStdString().data());
